...
def fetch_sec_ftd(ticker: str) -> Optional[dict]:
    """
    Fetch Failure-to-Deliver data from SEC EDGAR.

    FTD = shares where short sellers couldn't locate borrow.
    High FTD relative to float = real borrow scarcity.
    SEC publishes this twice monthly (free, public data).

    Returns dict with:
      ftd_shares      - total FTD shares in most recent report
      ftd_pct_float   - FTD shares / float shares
      report_date     - date of most recent data
      data_quality    - 'live' / 'error'
    """
    result = {'ftd_shares': None, 'ftd_pct_float': None,
               'report_date': None, 'data_quality': 'error'}
    try:
        # SEC FTD files are published at the start of each month
        # for the prior settlement period
        now = datetime.now()
        year = now.year
        month = now.month
        ...
        for attempt in range(3):
            d = now - timedelta(days=attempt * 30)
            yr, mo = d.year, d.month
            # SEC uses YYYYMM format, two files per month (1st and 2nd half)
            for half in ['02', '01']:
                url = (
                    f"https://www.sec.gov/data/foiadocuments/"
                    f"fails-deliver-data/cnsfails{yr}{mo:02d}{half}.zip"
                )
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        with zipfile.ZipFile(io.BytesIO(resp.content)) as z:
                            ...
                            with z.open(fname) as f:
                                import pandas as pd
                                # SEC FTD format: DATE|CUSIP|TICKER|QUANTITY|DESCRIPTION|PRICE
                                ...
                                qty_col = next((c for c in df.columns
                                                if 'QUANTITY' in c or 'SHARES' in c), None)
                                if sym_col and qty_col:
                                    df[qty_col] = pd.to_numeric(df[qty_col], errors='coerce')
                                    ticker_rows = df[df[sym_col].str.upper() == ticker.upper()]
                                    if not ticker_rows.empty:
                                        total_ftd = ticker_rows[qty_col].sum()
                                        result['ftd_shares'] = int(total_ftd)
                                        result['report_date'] = f"{yr}-{mo:02d} ({half})"
...
def fetch_validated_info(ticker: str) -> dict:
    ...
    try:
        ...
        return {'_fetch_error': str(e), 'symbol': ticker}
...
def validate(value, min_val=None, max_val=None, default=None):
    """
    Validate a numeric value is within expected bounds.
    Returns the value if valid, default (None) if not.
    Used by analyzer files to sanity-check fetched data.
    """
    if value is None:
        return default
    try:
